...
const LOCAL_KEY = 'paper_chat_sessions';

function getLocalSessions(): PaperChatSession[] {
  try {
...
function saveLocalSessions(sessions: PaperChatSession[]) {
...
export function usePaperChatPersistence() {
  const { user } = useAuth();
  ...
  const loadSessions = useCallback(async () => {
    if (user) {
      ...
      if (data) {
        const sessionsWithMessages: PaperChatSession[] = await Promise.all(
          data.map(async (s) => {
    ...
    (async () => {
      for (const session of localSessions) {
        ...
        if (newSession) {
          ...
          await supabase.from('paper_chat_messages').insert(msgs);
        }
      }
  ...
  const createSession = useCallback(async (): Promise<string> => {
    ...
    const newSession: PaperChatSession = {
      id,
      title: 'New Chat',
  ...
  const saveMessageToDB = useCallback(async (sessionId: string, message: Message) => {
    if (!user) return;
    await supabase.from('paper_chat_messages').insert({
  ...
  const updateSessionTitle = useCallback(async (sessionId: string, title: string) => {
    if (user) {
      await supabase.from('paper_chat_sessions').update({ title }).eq('id', sessionId);
    }
  ...
  const deleteSession = useCallback(async (sessionId: string) => {
    if (user) {
      await supabase.from('paper_chat_sessions').delete().eq('id', sessionId);
    }
